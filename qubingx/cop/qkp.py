...
import numpy as np
from cop.base import QUBO, Encoding
...
class QKP(QUBO):
    def __init__(
        ...
        if encoding == Encoding.one_hot.value:
            super().__init__(
                ...
                q_all=np.zeros((num_item + capacity, num_item + capacity)),
                q_obj=np.zeros((num_item + capacity, num_item + capacity)),
                q_constraint=np.zeros((num_item + capacity, num_item + capacity)),
                offset_all=0,
                offset_obj=0,
                offset_constraint=0,
            )
        ...
        self.h_obj(num_item=num_item, values=values)
        self.h_constraint(
            encoding=encoding,
            num_item=num_item,
            weights=weights,
    ...
    def h_obj(self, num_item: int, values: list[list[int]]):
        # Cost term
